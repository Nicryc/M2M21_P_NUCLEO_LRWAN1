...
static void Callback1(sSendDataBinary_t *SendDataBinary);
static void Alarm_activated(sSendDataBinary_t *SendDataBinary);

/* load call backs*/
static LoRaDriverCallback_t LoRaDriverCallbacks = { SensorMeasureData,Callback1,Alarm_activated,
                                                    NULL
                                                  };
...
int main(void)
{
  ...
  DBG_PRINTF("before idle \n");
    ...
    HAL_GPIO_Init(USER_BUTTON_GPIO_PORT, &gpioinitstruct);
   //HW_GPIO_SetIrq(USER_BUTTON_GPIO_PORT, USER_BUTTON_PIN, 0, blink_led);
    //HW_GPIO_SetIrq(USER_BUTTON_GPIO_PORT, USER_BUTTON_PIN, 0, blink_led);
    //BSP_LED_Modem_On(LED_GREEN);
  /* Infinite loop */
  while (1)
  {
    /* run the LoRa Modem state machine*/
	    /*if(!HW_GPIO_Read(USER_BUTTON_GPIO_PORT, USER_BUTTON_PIN)){
	               lora_setDeviceState(DEVICE_BUTTON_PRESSED);
	                BSP_LED_Modem_On(LED_GREEN);
	    }
	    else{
	                BSP_LED_Modem_Off(LED_GREEN);
...
static void SensorMeasureData(sSendDataBinary_t *SendDataBinary)
{
  ...
  uint16_t humidity = 0;
  uint32_t BatLevel = 0;                /*end device connected to external power source*/              /*just for padding*/
  uint8_t index = 0;
...
static void Callback1(sSendDataBinary_t *SendDataBinary)
{
  ...
  uint8_t ButtonState = 0;                /*just for padding*/
  uint8_t index = 0;
  ATEerror_t LoraCmdRetCode;
  uint8_t cchannel = 3;
...
static void Alarm_activated(sSendDataBinary_t *SendDataBinary)
{
  uint8_t LedState = 50;
  uint32_t BatLevel = 0;                /*end device connected to external power source*/
  uint8_t ButtonState = 20;                /*just for padding*/
  uint8_t index = 0;
  ATEerror_t LoraCmdRetCode;
  uint8_t cchannel = 3;
